#+STARTUP: overview
#+TITLE: Init File in Literate Programming Style
* Introduction
  This configuration file contains setup of Emacs packages for Language Server Protocol (LSP). I will use ~use-package~ for package management. Refer [[https://xpressrazor.wordpress.com/2020/11/04/java-programming-in-emacs/][Xpressrazor blog]]
* Setup
...
#+begin_src emacs-lisp
  ...
  (setq org-agenda-files (list "~/Library/Mobile Documents/com~apple~CloudDocs/org/work.org"
							   "~/Library/Mobile Documents/com~apple~CloudDocs/org/personal.org"
							   "~/Library/Mobile Documents/com~apple~CloudDocs/org/events.org"))
...
** Install LSP Package
Let's install the main package for lsp. Here we will integrate lsp with which-key. This way, when we type the prefix key ~C-c l~ we get additional help for completing the command. Also add hook for java and golang to start lsp in deferred mode
...
:hook (
   ...
   ((java-mode go-mode ruby-mode) . #'lsp-deferred)
)
:init (setq 
...
Remove advice-add and init if there are issues
#+begin_src  emacs-lisp
  (use-package lsp-java 
	...
	:init (setq lsp-java-vmargs '("-XX:+UseParallelGC" "-XX:GCTimeRatio=4" "-XX:AdaptiveSizePolicyWeight=90" "-Dsun.zip.disableMemoryMapping=true" "-Xmx2G" "-Xms100m")
				))
#+end_src
...
* Custom Functions & Keyboard bindings
** Custom functions
*** Open Skyscanner Clock (renamed to work.org) file - TODO list
#+begin_src  emacs-lisp
(defun my-work ()
  "Open the clock file."
  (interactive)
  (find-file "~/Library/Mobile Documents/com~apple~CloudDocs/org/work.org"))
#+end_src
*** Open personal todo list in org
#+begin_src  emacs-lisp
(defun my-personal ()
  "Open personal todo"
  (interactive)
  (find-file "~/Library/Mobile Documents/com~apple~CloudDocs/org/personal.org"))
#+end_src

*** Open emacs shortcut key file
#+begin_src  emacs-lisp
(defun my-key ()
  "Open the emacs keyboard shortcut file."
  (interactive)
  (find-file "~/Library/Mobile Documents/com~apple~CloudDocs/skyscanner/emacs.org"))
#+end_src
...
(defun my-events ()
  "Open the emacs keyboard shortcut file."
  (interactive)
  (find-file "~/Library/Mobile Documents/com~apple~CloudDocs/org/events.org"))
#+end_src
...
(global-set-key (kbd "C-c c") 'gptel-send)
#+end_src
